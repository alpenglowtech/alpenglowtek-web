# Translation Script for Alpenglow Tek Website
# Translates Chinese pages to English and Japanese

$basePath = "g:\我的雲端硬碟\臻至科技工作檔案\02-業務發展資料\2-APG新版網站\alpenglowtek-web"

# ── English translations (zh → en) ──
$enMap = @{
    'lang="zh-TW"' = 'lang="en"'
    'lang="zh"' = 'lang="en"'
    # ANIX tech page
    'ANIX 技術介紹 — 超低功耗微型 AI 推論核心 | 臻至科技 Alpenglow Tek' = 'ANIX Technology — Ultra-Low Power Micro AI Inference Core | Alpenglow Tek'
    'ANIX 重新思考資料搬移與運算分工，為低功耗微型 AI 提供架構級解決方案。了解 ANIX 的核心技術與 FPGA 原型驗證成果。' = 'ANIX rethinks data movement and computation allocation, delivering architecture-level solutions for low-power micro AI. Explore the core technology and FPGA prototype validation results.'
...
function Translate-File {
    ...
    $sorted = $TranslationMap.GetEnumerator() | Sort-Object { $_.Key.Length } -Descending
...
# Translate English pages
$enPages = Get-ChildItem -Path (Join-Path $basePath "en") -Recurse -Filter "*.html"
Write-Host "=== Translating EN pages ==="
foreach ($f in $enPages) {
    Translate-File -FilePath $f.FullName -TranslationMap $enMap
}

# Translate Japanese pages
$jaPages = Get-ChildItem -Path (Join-Path $basePath "ja") -Recurse -Filter "*.html"
Write-Host "=== Translating JA pages ==="
foreach ($f in $jaPages) {
    Translate-File -FilePath $f.FullName -TranslationMap $jaMap
}
